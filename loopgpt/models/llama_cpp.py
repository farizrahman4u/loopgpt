...
class LlamaCppModel(BaseConversationModel):
    ...
    def chat(
        ...
        with tempfile.NamedTemporaryFile(mode='w', ) as fp:
            ...
            arguments = [llama_bin, "--file", fp.name, "--temp", str(temperature), "-n", str(max_tokens), "-c", str(self.get_token_limit())]
            if self.model:
    ...
    def encode_messages(self, messages: List[Dict[str, str]]):
        message_format = {
            ...
            "openassistant": { "system": "{0}\n", "user": "<|prompter|>{0}<|endoftext|>", "assistant": "<|assistant|>{0}<|endoftext|>" },
        }[self.prompt_style]
